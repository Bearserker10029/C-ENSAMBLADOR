    ...
    if (base>1 && base<10){
        ...
        if (numero>0){
            ...
            if (esValido){
                ...
                scanf(" %d",&nuevaBase);
                if (nuevaBase>base && nuevaBase<100){
                    exponenteK = calcularExponenteK(base,nuevaBase);
                    if (exponenteK==0){
                        printf("No se pudo determinar un exponente k entero de la forma base^k = nuevaBase");
                    }
                    else{
                        nuevoNum = cambiarBase(numero,exponenteK,base);
                    ...
                    if (nuevaBase<=base){
                        ...
                        printf("La nueva base no puede ser mayor o igual que 100");
                    }
                }
            }
            else {
...
int cambiarBase(int numero,int exponenteK,int base){
    int nuevoNum, posicion, digitos, nuevoDigito, exponente, cifra;
    nuevoNum = 0;
    posicion = 0;
    while (numero>0){
        digitos = numero % (int)pow(10,exponenteK);
        numero = numero/((int)pow(10,exponenteK));
        nuevoDigito = 0;
        exponente = 0;
        while (digitos>0){
            cifra = digitos %10;
            digitos = digitos/10;
            nuevoDigito = nuevoDigito + cifra*(int)pow(base,exponente);
            exponente++;
        }
        nuevoNum = nuevoNum + nuevoDigito*(int)pow(10,posicion);
        posicion++;
    }
    return nuevoNum;
}
